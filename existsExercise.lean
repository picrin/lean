...
lemma doubleNegateRev {r : Prop} : ¬ ¬ r → r :=
    assume (Hr : ¬ ¬ r),
        ...
        or.elim J (λ (H : r), H) (λ (H : ¬ r), false.elim (Hr H))
...
example : (∃ x, p x ∧ r) ↔ (∃ x, p x) ∧ r := iff.intro
    (assume (HexistsTogether : (∃ x, p x ∧ r)),
    exists.elim HexistsTogether
        (assume a : A,
            (assume H : p a ∧ r,
                and.intro
                    (exists.intro a (and.elim_left H))
                    (and.elim_right H))))
    (assume (HexistSeparately : (∃ x, p x) ∧ r),
            exists.elim (and.elim_left HexistSeparately)
